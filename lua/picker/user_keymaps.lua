...
---@type table[]
local specs = {
  { mode = "n", lhs = "<leader>fF", desc = "Find Files (root)", action = function()
    local p = picker()
  ...
  { mode = "n", lhs = "<leader>ff", desc = "Find Files (cwd)", action = function()
    picker().find_files({ title = "Find Files (cwd)", input_mode = true, layout = "intellij_grep" })
  end },
  { mode = "n", lhs = "<leader>fi", desc = "Find Ignored Files (cwd)", action = function()
    picker().find_files({ ignored = true, title = "Find Ignored Files (cwd)" })
  end },
  { mode = "n", lhs = "<leader>fI", desc = "Find Ignored Files (root)", action = function()
    local p = picker()
    p.find_files({ cwd = p.root(), ignored = true, title = "Find Ignored Files (root)" })
  end },
  { mode = "n", lhs = "<leader>fg", desc = "Find Git Files", action = function()
    picker().git_files({ title = "Find Git Files" })
  end },
  { mode = "n", lhs = "<leader>fR", desc = "Recent Files (cwd)", action = function()
  ...
  { mode = "n", lhs = "<leader>sc", desc = "Command History", action = function() picker().command_history() end },
  ...
  { mode = { "n", "x" }, lhs = "<leader>sw", desc = "Search Word (cwd)", action = function() picker().grep_word({}) end },
  { mode = "n", lhs = "<leader>/", desc = "Fast search text in current file (rg)", action = function() picker().grep_buffer() end },
  { mode = "n", lhs = "<leader>bd", desc = "Delete Buffer", action = function() picker().delete_buffer() end },
  { mode = "n", lhs = "<leader>bo", desc = "Delete Other Buffers", action = function() picker().delete_other_buffers() end },
  { mode = "n", lhs = "<leader>gG", desc = "Lazygit (root)", condition = function() return vim.fn.executable("lazygit") == 1 end, action = function()
    local p = picker()
    p.lazygit(p.root())
  end },
  { mode = "n", lhs = "<leader>gg", desc = "Lazygit (cwd)", condition = function() return vim.fn.executable("lazygit") == 1 end, action = function()
    picker().lazygit(vim.fn.getcwd())
  end },
  { mode = "n", lhs = "<leader>gl", desc = "Git Log (cwd)", action = function() picker().git_log(vim.fn.getcwd()) end },
  { mode = "n", lhs = "<leader>gb", desc = "Git Blame Line", action = function() picker().git_blame_line() end },
  { mode = "n", lhs = "<leader>gf", desc = "Git File History", action = function() picker().git_file_history() end },
  { mode = "n", lhs = "<leader>gL", desc = "Git Log (root)", action = function() picker().git_log(picker().root()) end },
